...
/// Get articles.json
const getNews = require("../public/json/articles.json");
...
router.get("/",(request, response) =>{
  response.render("index", {getNewsList, getNews});
});

router.get("/actualite",(request, response) =>{
  response.render("actualite", {getNewsList, getNews});
});
...
/// Les actualités
router.route("/actualite/:news",(request, response) =>{
  const getTitleNews = request.params.news;
  response.send("modelActualite", {getNews, getTitleNews});
});
